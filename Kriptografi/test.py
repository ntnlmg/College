...
Plaintext = "411bc210f277bf8550650d30399e9389"
# Key = input("Please enter the Key (max = %d): " % n_bits)
Key = "241e64fa9136825d7c664516148e1951"
n_bits = 128
Round = 2
...
def AES(Plaintext, Key, n_bits, Round):
    ...
    def SubBytes(A):  # replace bits by S-box
        ...
        print("Sbox ", Sbox[int(A[0], 16)][int(A[1], 16)])
    ...
    def KeySchedule(Key, RoundNum):
       # compose the Rcon[0][0]=RC[Round] of the Round round
        ...
        for i in range(0, 4):
            for j in range(0, 4):
                print('ij ', i, j, 'value ', W[i][j][0], W[i][j][1])
                W[i][j] = SubBytes(W[i][j])
        print(W, '\Roundcon last column')
...
# Main code
while len(Plaintext) % (n_bits//4) != 0:     # fill 32-bit block with '0'
    Plaintext += '0'

for i in range(round(len(Plaintext) / (n_bits//4))):
    AES(Plaintext[i * (n_bits//4):(i + 1)
        * (n_bits//4)], Key, n_bits, Round)
    print('--------------------------------------------------------------')
